...
export const createCameraPage = (): HTMLElement => {
  ...
  page.querySelector('#camera-start')?.addEventListener('click', async () => {
    ...
    const response = await BrowserCamera.turnOnCamera(readConstraints());
  ...
  page.querySelector('#camera-stop')?.addEventListener('click', () => {
    const stopped = BrowserCamera.turnOffCamera();
  ...
  page.querySelector('#camera-devices')?.addEventListener('click', async () => {
    ...
    result.textContent = devices.length
      ? devices
          .map((device) => `${device.label || 'Unnamed camera'} (${device.deviceId})`)
          .join('\n')
  ...
  page.querySelector('#camera-record-stop')?.addEventListener('click', () => {
    const recording = BrowserCamera.stopRecording();
