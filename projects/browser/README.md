...
- `Canvas`
  ...
  - `createSession`: create a `CanvasSession` with a supplied or new canvas element.

- `CanvasSession`
  - `getContext`, `getSize`, `resize`, `clear`: access and prepare the canvas.
  - `drawLine`, `drawRectangle`, `drawCircle`, `drawText`, `drawImage`: draw common shapes and images.
  - `getImageData`, `putImageData`: read and write pixel data.
  - `createGradient`, `drawPath`, `drawPolyline`: draw custom paths and gradients.
  - `rotate`, `scale`, `translate`, `flip`, `resetTransform`: control the drawing transform.
  - `toBlob`, `toDataUrl`: export the canvas content.

  ```ts
  import { Canvas } from '@trt-web/browser';

  const session = Canvas.createSession();
  session.resize({ width: 320, height: 180 });
  ...
  const image = await session.toBlob({ type: 'image/png' });
